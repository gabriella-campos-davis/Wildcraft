...
namespace wildcraft
{
    public class BlockEntityWCBerryBush : BlockEntity, IAnimalFoodSource
    {
        ...
        { 
            ...
            if(nowCodePart =="flowering"){
                BlockPos below = Pos.DownCopy(1);
                BlockPos above = Pos.UpCopy(1);
                Block blockBelow = Api.World.BlockAccessor.GetBlock(below);
                Block blockAbove = Api.World.BlockAccessor.GetBlock(above);

                bool growGrowth = true;

                if(block.Attributes?.IsTrue("stackable") == false){
                    growGrowth = false;
                }

                if(blockBelow.EntityClass == block.EntityClass){
                    growGrowth = false;
                }
                if(blockAbove.EntityClass == block.EntityClass){
                    growGrowth = false;
                }
                if(growGrowth == true){
